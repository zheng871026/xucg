...
static ucs_status_t ucg_apply_params(ucg_context_h context, const ucg_params_t *params)
{
    ...
    UCG_CHECK_REQUIRED_FIELD(field_mask, UCG_PARAM_FIELD_GLOBALIDX_CB, err);
    context->params.mpi_global_idx_f = params->mpi_global_idx_f;

    UCG_CHECK_REQUIRED_FIELD(field_mask, UCG_PARAM_FIELD_DTCONVERT_CB, err);
    context->params.mpi_dt_convert = params->mpi_dt_convert;

    UCG_CHECK_REQUIRED_FIELD(field_mask, UCG_PARAM_FIELD_DATATYPESPAN_CB, err);
    context->params.mpi_datatype_span = params->mpi_datatype_span;
                              ...
                              ucg_context_h *ctx_p)
{
    ...
    if (config == NULL) {
        status = ucg_config_read("UCX_", NULL, &dfl_config);
        if (status != UCS_OK) {
                                        ...
                                        unsigned *last_am_id)
{
    if (*last_am_id >= UCP_AM_ID_MAX) {
        ucs_error("active message id out-of-range (last_am_id: %d max: %d)", *last_am_id, UCP_AM_ID_MAX);
